...
**bs-be** is the Go backend for the BetterBankings cashflow analysis platform. It receives CSV uploads of loan portfolio data, validates them, computes amortization schedules, and distributes cashflows into regulatory time buckets (IRRBB, LCR, NSFR). It supports default behaviours, custom scenario behaviours (with 2-section CSV import), pivot table aggregation, Excel export, and a SuperAdmin reference-data management panel.
...
├── models/
│   ├── loan.go                 Loan struct (parsed from CSV), TenorDays/TenorMonths helpers
...
cashflow_results (id BIGSERIAL, upload_id FK CASCADE, loan_input_id FK CASCADE, remaining_days,
                  ...
                  nsfr_principal JSONB, nsfr_interest JSONB, result_type VARCHAR(100), behaviour_id FK)
```
...
- `validator.ValidateAndParseCSV()` — validates headers (with aliases), parses each row into `models.Loan`
...
- If `loan.HasEndDate()`: Generate amortization schedule → bucket into IRRBB/LCR/NSFR → result_type = `"Contractual"`
...
- `FindMatchingConfig()` — linear scan with wildcard matching (`"All"` or empty = matches everything)
- `ComputeScenarioBucketsForType()` — `baseValue × percentage × cashflowAssumption`
  - `ValueType`: "Outstanding" or "Market" determines the base value
- `ComputeAllScenarioBuckets()` — runs for IRRBB, LCR, NSFR; falls back to amortization/default when no matching config
...
| POST   | `/api/upload`                      | Upload CSV → validate → process (background)                         |
...
| POST   | `/api/uploads/:id/behaviours`      | Upload scenario CSV (2-section format)                               |
...
**Validation:** Overlap detection across both sections — if two rules match the same loan criteria for the same bucket type, it's rejected. `"All"` or empty values act as wildcards.
...
- **Interest Rate is divided by 100** in the validator — CSV contains percentages (e.g., 9%), stored as decimal (0.09)
...
- **Scenario overlap validator** — mirrors `installment_software/validator.py`, checks within Bucket section, within Cashflow section, and cross-section
